...
import { ShowProgressBadge } from '../ShowProgressBadge'
...
export const WatchlistItem = ({
  ...
  const { getShowProgress } = useCollection()
  const detailPath =
    entry.mediaType === 'movie' ? `/movies/${entry.mediaId}` : `/tv/${entry.mediaId}`

  const totalEpisodes = entry.snapshot.totalEpisodes ?? 0
  const showProgress =
    entry.mediaType === 'tv' && totalEpisodes > 0
      ? getShowProgress(entry.mediaId, totalEpisodes)
      : null
  ...
  return (
    <article className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-4 sm:flex-row dark:border-zinc-800 dark:bg-zinc-900">
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-3">
        ...
        <InlineNoteEditor
          value={entry.note}
          onSave={(note) => onNoteChange(entry.id, note)}
          onClear={() => onNoteClear(entry.id)}
        />
      </div>
    </article>
  )
